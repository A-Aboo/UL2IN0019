...
/*domaines*/
/* Le numéro de sécurité sociale possède exactement 5 chiffres.*/ create domain dnumss as numeric(5) check(length(value)=5);
...
/*Le salaire peut avoir deux chiffres après la virgule et ne dépasse pas 90 000.*/ create domain dsal as numeric(7,2) check (value <=90000);
...
/*EMPLOYE (NumSS, NomE, PrenomE, NumChef*, VilleE, DateNaiss)*/ create table Employe(
NumSS dnumss,
NomE dchaines,
PrenomE dchaines,
NumChef dnumss,
...
constraint fk_chef foreign key (numchef) references Employe, constraint key_emp unique(NomE, PrenomE) );
/*PROJET(NumProj, NomProj, RespProj*, VilleP, Budget)*/ create table Projet(
NumProj dnumproj,
NomProj dchaines,
RespProj dnumss,
VilleP dvilles,
...
/*GRILLE_SAL (Profil, salaire)*/ create table Grille_sal(
profil dchaines,
salaire dsal,
/*contraintes de cle et referentielles*/ constraint pk_sal primary key (profil) );
/*EMBAUCHE (NumSS*, NumProj*, DateEmb, Profil*)*/ create table Embauche(
NumSS dnumss,
NumProj dnumproj,
DateEmb date,
...
constraint fk_emb_emp foreign key (NumSS) references Employe, constraint fk_emb_proj foreign key (NumProj) references Projet, constraint fk_emb_sal foreign key (Profil) references Grille_sal
);
